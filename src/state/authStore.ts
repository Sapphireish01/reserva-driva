...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  deleteAccount: async () => {
    clearProactiveRefreshTimer();
    try {
      console.log("🌐 [API Call] POST /accounts/delete/ with refresh token...");
      const refreshToken = await getStoredRefreshToken();
      const res = await authService.deleteAccount(refreshToken || "");
      console.log("✅ [API Call] Account deleted successfully:", res.data);
      return res.data;
    } finally {
      await clearStoredTokens();
      set({ isAuthenticated: false, user: null });
